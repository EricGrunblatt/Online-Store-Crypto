            ...
            <div style={{ margin: '3% 0 3% 7%', display: 'grid', gridTemplateColumns: 'repeat(2, 35vw)' }}>
                {items.map((index, i) => (
                    <div key={index._id} style={{ marginBottom: '5%', display: 'grid', gridTemplateColumns: 'repeat(2, 35vw)', width: '95%', height: '200px', border: 'black 2px solid', borderRadius: '20px' }}>
						...
						{/* ITEM INFO */}
						<div style={{position: 'absolute', margin: '5px 0 0 200px'}}>
							...
							<div style={{ display: 'flex', marginTop: '3px', fontSize: '20px' }}>Seller: 
								<div onClick={() => { handleViewProfile(index.sellerUsername) }} style={{ cursor: 'pointer', marginLeft: '8px', color: '#879ED9' }}><u>{index.sellerUsername}</u></div>
							</div>
